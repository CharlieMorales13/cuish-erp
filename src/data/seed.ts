import type { Compra, Conteo, Lote, Movimiento, Proveedor, Venta } from '../domain/types'
import { INSUMOS } from './insumos'
import { RECETAS } from './recetas'
import { byId, costoReceta } from '../domain/inventario'
...
export const PROVEEDORES: Proveedor[] = [
  { id: 'PRV-01', nombre: 'Distribuidora La Noria', contacto: 'Elena Ruiz', telefono: '951 512 3344', cascosPrestados: 0 },
  { id: 'PRV-02', nombre: 'Cervecería Modelo Oaxaca', contacto: 'Sergio Nava', telefono: '951 514 8890', cascosPrestados: 10 },
  { id: 'PRV-03', nombre: 'Mezcales de Santiago Matatlán', contacto: 'Doña Rufina', telefono: '951 588 2211', cascosPrestados: 0 },
  { id: 'PRV-04', nombre: 'Central de Abastos — Frutas Ríos', contacto: 'Mostrador', telefono: '951 500 1177', cascosPrestados: 0 },
]
...
let n = 0
const nuevoLote = (p: Omit<Lote, 'id'>): Lote => ({ id: `LOT-${String(++n).padStart(3, '0')}`, ...p })
...
export const LOTES: Lote[] = INSUMOS.flatMap((insumo, idx) => {
  const proveedorId =
    insumo.categoria === 'Perecederos' ? 'PRV-04'
      : insumo.id === 'INS-01' ? 'PRV-03'
      : insumo.categoria === 'Mezcladores' ? 'PRV-02' : 'PRV-01'
...
export const COMPRAS: Compra[] = [
  {
    id: 'CMP-001', folio: 'CMP-001', proveedorId: 'PRV-02', fecha: fecha(6), estado: 'recibida',
    lineas: [
      { insumoId: 'INS-19', presentaciones: 24, costoCompra: 13.33 },
      { insumoId: 'INS-20', presentaciones: 24, costoCompra: 16.0 },
    ],
    cascosPrestados: 10, cascosDevueltos: 0,
  },
  {
    id: 'CMP-002', folio: 'CMP-002', proveedorId: 'PRV-03', fecha: fecha(3), estado: 'recibida',
    lineas: [{ insumoId: 'INS-01', presentaciones: 6, costoCompra: 150 }],
    cascosPrestados: 0, cascosDevueltos: 0,
  },
  {
    id: 'CMP-003', folio: 'CMP-003', proveedorId: 'PRV-01', fecha: fecha(0), estado: 'requisicion',
    lineas: [
...
const cuentas = ['Barra principal 1', 'Barra principal 2', 'Barra pared 1', 'Mesa 1', 'Barra pared 2']
...
export const CONTEOS: Conteo[] = [
  {
    id: 'CTO-001', folio: 'CTO-001', fecha: fecha(7), estado: 'cerrado',
    lineas: INSUMOS.slice(0, 8).map((i) => ({ insumoId: i.id, teorico: i.presentacion, fisico: i.presentacion })),
  },
]

/** Costo de producción calculado por receta, para el dashboard. */
export const costoCalculado = (recetaId: string) => costoReceta(recetas[recetaId], insumos)
